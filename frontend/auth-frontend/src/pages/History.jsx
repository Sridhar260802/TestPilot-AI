...
// Pulls the PDF for one history row and saves it to the user's disk.
...
async function downloadHistoryReport(entryId, plan) {
  ...
  a.href = url;
  a.download = `TestPilot_${plan ? plan[0].toUpperCase() + plan.slice(1) : "Website"}_Report_${entryId}.pdf`;
...
export default function History() {
  ...
  const filtered = useMemo(() => {
    return entries.filter((e) => {
      const matchesQuery = query.trim() === "" || e.url.toLowerCase().includes(query.trim().toLowerCase());
    ...
    <div className="min-h-screen bg-[#F1ECDF] text-[#14181B]">
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-12 sm:px-6">
        ...
        <p className="mt-2 text-sm text-[#14181B]/55">
          Every website you've run through Crosbytech, most recent first.
        </p>
          ...
          {!loading && !errored && filtered.length === 0 && (
            <div className="p-10 text-center">
              ...
              <p className="mt-1 text-xs text-[#14181B]/50">
                {entries.length === 0
                  ? "Run your first test and it'll show up here."
