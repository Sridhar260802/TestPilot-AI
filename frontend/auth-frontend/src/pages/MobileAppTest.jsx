...
export default function MobileAppTest() {
  ...
  // TEMP: while Razorpay isn't wired up, let a no-plan user (e.g. fresh
  // Google sign-in) pick a plan locally just to run a scan. Not saved to
  // their account - backend currently allows any logged-in user on any
  // plan endpoint (see TEMP_ALLOW_ALL_PLANS_NO_PAYMENT in app/core/plans.py).
  ...
  async function handleSubmit(e) {
    ...
    try {
      const analysis = await runMobileTest(file, plan);
      setResult(analysis);
      setStatus("done");
    } catch (err) {
      setStatus("error");
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#F1ECDF] text-[#14181B]">
      ...
      {!plan ? (
        <div className="animate-fade-in-up relative z-10 mx-auto max-w-xl px-4 py-24 text-center sm:px-6">
          ...
          <button
            ...
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#14181B] px-7 py-3.5 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#E4572E] hover:shadow-lg active:scale-[0.97] active:translate-y-0"
          >
            View pricing
          </button>

          {/* TEMP: payments aren't live yet - let the user try any tier's
              scan right now. Local selection only, nothing charged/saved. */}
          <p className="mt-8 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#14181B]/35" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
          ...
          <ul className="mt-3 flex flex-wrap items-center justify-center gap-2">
            {["basic", "standard", "premium"].map((p, i) => (
              ...
              >
                <button
                  type="button"
                  onClick={() => setTempPlan(p)}
                  className="rounded-full border border-[#14181B]/12 bg-white px-3.5 py-1.5 text-xs font-medium capitalize text-[#14181B]/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#1F5C45]/40 hover:text-[#1F5C45] hover:shadow-sm active:scale-[0.97]"
                >
                  {p}
                </button>
              </li>
        ...
        <div className="relative z-10 mx-auto max-w-2xl px-4 py-14 sm:px-6">
          <div className="animate-fade-in-up flex items-center gap-2">
            ...
            >
              <span className="h-1.5 w-1.5 rounded-full bg-[#1F5C45] animate-pulse" />
              {copy.label} plan
            </span>
            <span
          ...
          <h1
            className="animate-fade-in-up mt-4 text-3xl font-semibold leading-tight tracking-tight sm:text-[2.2rem]"
            style={{ fontFamily: "'Space Grotesk', sans-serif", animationDelay: "0.05s" }}
          >
            {copy.headline}
          </h1>
          <p
            className="animate-fade-in-up mt-3 max-w-md text-sm leading-relaxed text-[#14181B]/60"
          ...
          {status !== "done" && (
            ...
            >
              ...
              <div
                className={`flex flex-col gap-3 rounded-sm border bg-white px-4 py-4 transition-all duration-300 sm:flex-row sm:items-center ${
                  status === "running"
                    ...
                    : "border-[#14181B]/15 hover:border-[#14181B]/30 hover:shadow-sm"
                }`}
              >
                <input
                  ref={fileInputRef}
                  ...
                  className="w-full text-sm text-[#14181B]/70 file:mr-3 file:rounded-sm file:border-0 file:bg-[#14181B] file:px-4 file:py-2 file:text-xs file:font-semibold file:text-white file:transition-colors file:duration-200 hover:file:bg-[#E4572E] disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={status === "running" || !file}
                  className="shrink-0 rounded-sm bg-[#14181B] px-5 py-2.5 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#E4572E] hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none active:scale-[0.97] active:translate-y-0"
                >
              ...
              {file && (
                <p
                  className="animate-fade-in mt-2 truncate text-xs text-[#14181B]/45"
                  style={{ fontFamily: "'IBM Plex Mono', monospace" }}
                >
          ...
          {status === "running" && (
            <div className="animate-fade-in-up mt-6 flex items-center gap-3 rounded-sm border border-[#1F5C45]/25 bg-white px-5 py-4 text-sm">
          ...
          {status === "error" && (
            <div role="alert" className="animate-shake animate-fade-in-up mt-6 rounded-sm border border-[#E4572E]/35 bg-white px-5 py-4 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <button
                    type="button"
                    onClick={handleRetry}
                    className="mt-3 inline-flex items-center gap-1.5 rounded-sm border border-[#14181B]/20 bg-white px-3.5 py-1.5 text-xs font-semibold text-[#14181B] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#E4572E] hover:text-[#E4572E] active:scale-[0.97] active:translate-y-0"
                  >
                    Try again
                  </button>
                </div>
...
function ScoreRing({ score, severity }) {
  ...
  // Starts at 0 and eases up to the real score on mount, so the ring
  // (and the number ticking alongside it) visibly fills in rather than
  // popping straight to the final value.
...
function ResultCard({ result, downloadState, onDownload }) {
  ...
  return (
    <div className="animate-fade-in-up overflow-hidden rounded-xl border border-[#14181B]/10 bg-white shadow-[0_1px_0_rgba(20,24,27,0.03)] transition-shadow duration-300 hover:shadow-md">
      <div className="flex items-center gap-4 border-b border-[#14181B]/8 bg-[#14181B]/[0.02] px-6 py-5">
        ...
        {result.id && (
          <button
            ...
            className="shrink-0 rounded-sm bg-[#E4572E] px-4 py-2 text-xs font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#F16A40] hover:shadow-md disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:shadow-none active:scale-[0.97] active:translate-y-0"
          >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 px-6 py-4 text-xs sm:grid-cols-3">
        {Object.entries(overview).slice(0, 6).map(([key, value], i) => (
          <div
            key={key}
            className="animate-fade-in-up truncate"
          ...
          <ul className="mt-3 space-y-2">
            {issues.map((issue, i) => {
              ...
              return (
                <li
                  key={i}
                  className={`animate-fade-in-up rounded-sm border ${s.ring} ${s.bg} px-3.5 py-2.5 transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-sm`}
        ...
        <div className="border-t border-dashed border-[#14181B]/10 px-6 py-5">
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {permissions.map((p, i) => (
              <li
                key={p.permission}
                className="animate-fade-in-up rounded-sm border border-[#14181B]/12 px-3 py-1.5 text-xs font-medium text-[#14181B]/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#14181B]/25 hover:shadow-sm"
          ...
          <ul className="mt-3 space-y-1.5 text-xs">
            {Object.entries(exported).flatMap(([type, items]) =>
              items.map((c, i) => (
                <li
                  key={`${type}-${c.name}`}
                  className="animate-fade-in-up flex items-center justify-between rounded-sm border border-[#14181B]/10 px-3 py-1.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#14181B]/25 hover:shadow-sm"
      ...
      {(secretScan || cryptoScan) && (
        <div className="animate-fade-in-up border-t border-dashed border-[#14181B]/10 px-6 py-5">
          <p
            className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#14181B]/40"
            style={{ fontFamily: "'IBM Plex Mono', monospace" }}
          >
